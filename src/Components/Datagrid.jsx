...
function Datagrid() {
  ...
  // ✅ Only store plain objects in state
  ...
  const handleAdd = () => {
    const item = { ...newData, id: data.length + 1 };
    setData([...data, item]);
    setNewData({ id: null, name: "", dob: "", occupation: "" }); // clear input
  };
        ...
        <tbody>
          ...
          {data.map((row, index) => (
            <tr key={row.id}>
              ...
              <td>
                {editIndex === index
                  ? <input type="date" value={newData.dob} onChange={(e) => setNewData({ ...newData, dob: e.target.value })} />
                  : row.dob}
              </td>
              <td>
                {editIndex === index
                  ? <input type="text" value={newData.occupation} onChange={(e) => setNewData({ ...newData, occupation: e.target.value })} />
                  : row.occupation}
              </td>
              <td>
                {editIndex === index ? (
                  <Stack spacing={2} direction="row" sx={{ display: 'flex', margin: '1rem' }}>
                    ...
                    <ColorButton onClick={() => setEditIndex(-1)} variant="contained" sx={{ flex: 1, backgroundColor: 'red' }}>Cancel</ColorButton>
                  </Stack>
                ) : (
                  <Stack spacing={2} direction="row" sx={{ display: 'flex', margin: '1rem' }}>
